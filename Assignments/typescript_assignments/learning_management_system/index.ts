...
import {Student} from "./data/Types.js";
    ...
    switch (studentMenuResp.studentMenuItem) {
        case 'Add new student':
            ...
            console.log(chalk.green(
                `Successfully registered ${addStudentResp.student_Name} with Roll No ${addStudentResp.Roll_No} in the course ${addStudentResp.link_Course_Name}`
            ));
            await studentMenu();
            break;
        case 'Edit student':
            ...
            const selected_student = students_db.find(stu =>
                stu.name + "(" + stu.rollNo + ")" === ans.student_edit
            )!!;
            ...
            const studentNewCourse = courses_db.find(course =>
                course.name === resp.new_link_Course_Name
            )!!;
            const finalStudent = {...selected_student, name: resp.new_student_Name, courseId: studentNewCourse.id}
            const oldIndex: number = students_db.findIndex(stu =>
                stu.rollNo === selected_student?.rollNo
            );
        ...
        case 'Delete student':
            ...
            const selectedStudentIndex = students_db.findIndex(stu =>
                stu.name + "(" + stu.rollNo + ")" === deleteAns.student_delete
            );
